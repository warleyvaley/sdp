...
{
//alert('DynarchMenu._documentMouseDown');	
	ev||(ev=window.event);
...
DynarchMenu.prototype.createMenuTree=function(ul,horiz)
{
	...
	info=new DynarchMenu.MenuTree(
		{
			menu:this,base:base,horiz:horiz,element:ret,active_submenu:null,active_item:null,visible:false,keymap:{},config:cfg,_T_close:null,open_left:DynarchMenu._RE_OPL.test(ul.className)
		}
	);
	if(ul.id)
		info.id=ul.id;
...
//xun
//alert( el.tagName + " " + r.x + " " + r.y + " " + SL + " " + ST );
	return r;
};
...
DynarchMenu.addInfo=function(el,name,value)
{
	...
	if(is_ie)
	{
		...
		if(!a)
		{
			...
			DynarchMenu._addEvent(_dynarch_top,"unload",_dynarch_top.DynarchMenu._cleanUp);	//xun IE needs cleanup
...
DynarchMenu.DefaultAction=function(li)
{
	...
	{
		if(li.onclick)		//xun: causes problem if using setAttribute( "onclick", ... ) when the caller dynamically creates the ul list
		{
...
DynarchMenu.EventHandlers=
{
	popup_resetActive:function(ev)
		...
		{
//alert( "itemOver " + ev.target.tagName + " " + this.tagName );
			ev||(ev=window.event);
		...
		,item_onMouseDown:function(ev)		//xun for top and sub item both
		{
//alert('item_onMouseDown');
			ev||(ev=window.event);
			var info=this.__msh_info,ret;
//xun
//alert( ev.target.tagName + " " + this.tagName );
...
//xun
//tree mousedown and item mousedown both receive the events
//but after setting stopPropagation, tree mousedown won't receive the event
			DynarchMenu._stopEvent(ev);
		...
		{
			ev||(ev=window.event);
//document.getElementById("xunlai").value += "treeOver " + ev.target.tagName + " " + this.tagName + " " + ev.relatedTarget.tagName + " " + (ev.target.tagName==this.tagName) + " " + this.__msh_info.parent + " " + DynarchMenu._related(this,ev) + "\n";
			if(!DynarchMenu._related(this,ev))
			{
				var info=this.__msh_info;
				if(info.parent)
				{
//document.getElementById("xunlai").value += " arrived \n";					
//alert( "treeOver " + ev.target.tagName + " " + this.tagName + " " + (ev.target.tagName==this.tagName) + " " + this.__msh_info.parent + " " + DynarchMenu._related(this,ev) );
...
DynarchMenu.MenuTree.prototype._open=function(el,item,pos)
{
	...
	var m=this.element,
		self=this.menu,
		cfg=self.config,win=cfg.frames.popups,
		p=el?win.DynarchMenu._getPos(el):pos,
		pe,
		base=this.parent?this.parent.base:false,
		dx=base?cfg.basedx:cfg.dx,
		dy=base?cfg.basedy:cfg.dy,horiz=this.parent?this.parent.horiz:false,tmp,s,vw,sw;
//xun
//alert( el.tagName );		// td if base&horizontal, otherwise tr
